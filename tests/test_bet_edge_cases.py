...
class TestBetEdgeCases(unittest.TestCase):
    ...
    def test_dozen_bets_with_all_boundary_numbers(self):
        ...
        for space_val, d1_win, d2_win, d3_win in test_cases:
            ...
            self.assertEqual(
                dozen1.is_winning_bet(space), d1_win, f"Dozen 1 failed for {space_val}"
            )
            self.assertEqual(
                dozen2.is_winning_bet(space), d2_win, f"Dozen 2 failed for {space_val}"
            )
            self.assertEqual(
                dozen3.is_winning_bet(space), d3_win, f"Dozen 3 failed for {space_val}"
            )
    ...
    def test_column_bets_with_all_numbers(self):
        ...
        for space_val, c1_win, c2_win, c3_win in test_cases:
            space = Space(space_val)
            self.assertEqual(
                col1.is_winning_bet(space), c1_win, f"Column 1 failed for {space_val}"
            )
            self.assertEqual(
                col2.is_winning_bet(space), c2_win, f"Column 2 failed for {space_val}"
            )
            self.assertEqual(
                col3.is_winning_bet(space), c3_win, f"Column 3 failed for {space_val}"
            )
    ...
    def test_all_payout_ratios_exist(self):
        ...
        for bet_type in BetType:
            self.assertIn(
                bet_type, Bet.PAYOUT_RATIOS, f"Payout ratio missing for {bet_type}"
            )
    ...
    def test_bet_string_methods_with_different_data(self):
        ...
        corner_bet = Bet(
            BetType.CORNER, ["1", "2", "4", "5"], amount=20, layout=self.layout
        )
