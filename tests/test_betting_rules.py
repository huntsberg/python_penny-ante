...
"""
    ...
    def test_house_edge_calculation(self):
        ...
        self.assertAlmostEqual(
            rules.get_house_edge(BetType.STRAIGHT_UP), 5.26, places=2
        )
        self.assertAlmostEqual(rules.get_house_edge(BetType.RED), 5.26, places=2)

        # Test European table (37 pockets)
        rules_european = BettingRules(
            config_path=self.config_path, table_type="EUROPEAN"
        )
        self.assertAlmostEqual(
            rules_european.get_house_edge(BetType.STRAIGHT_UP), 2.70, places=2
        )
        self.assertAlmostEqual(
            rules_european.get_house_edge(BetType.RED), 2.70, places=2
        )
    ...
    def test_bet_amount_validation(self):
        ...
        # Valid amounts
        self.assertTrue(
            rules.validate_bet_amount(BetType.STRAIGHT_UP, 50)
        )  # Between 1 and 50000
        self.assertTrue(
            rules.validate_bet_amount(BetType.RED, 100)
        )  # Between 50 and 100000

        # Invalid amounts - too low (red minimum is now 50, not 10)
        self.assertFalse(
            rules.validate_bet_amount(BetType.RED, 25)
        )  # Below minimum of 50

        # Invalid amounts - too high
        self.assertFalse(
            rules.validate_bet_amount(BetType.STRAIGHT_UP, 60000)
        )  # Above max of 50000
...
"""
    ...
    def test_different_table_types(self):
        ...
        american_rules = BettingRules(
            config_path=self.config_path, table_type="AMERICAN"
        )
        european_rules = BettingRules(
            config_path=self.config_path, table_type="EUROPEAN"
        )

        # Both should have same table limits from config
        self.assertEqual(
            american_rules.get_table_minimum(), european_rules.get_table_minimum()
        )
        self.assertEqual(
            american_rules.get_table_maximum(), european_rules.get_table_maximum()
        )

        # But different calculated house edges
        self.assertAlmostEqual(
            american_rules.get_house_edge(BetType.RED), 5.26, places=2
        )
        self.assertAlmostEqual(
            european_rules.get_house_edge(BetType.RED), 2.70, places=2
        )
