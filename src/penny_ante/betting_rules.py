...
class BettingRules:
    ...
    def _apply_overlay(
        self, base_config: Dict[str, Any], overlay_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _get_winning_outcomes(self, bet_type: "BetType") -> int:
        ...
        # Default fallback
            ...
            raise ValueError(
                f"Unknown bet type for house edge calculation: {bet_type_str}"
            )
    ...
    def _validate_special_rules(
        self, bets: List["Bet"], validation_result: Dict[str, Any]
    ) -> None:
        """
        ...
        if not self.special_rules.get("progressive_betting", True):
            ...
            validation_result["warnings"].append(
                "Progressive betting systems are not allowed on this table"
            )
        ...
        if not self.special_rules.get("allow_call_bets", False):
            ...
            for i, bet in enumerate(bets):
                if hasattr(bet, "is_call_bet") and getattr(bet, "is_call_bet", False):
                    validation_result["valid"] = False
                    validation_result["errors"].append(
                        f"Bet {i+1}: Call bets are not allowed on this table"
                    )
